...
/// ## Commands
///
/// ```cargo run -q -p rust-in-action-types-array_bin --bin rust-in-action-types-array-main```
///
/// ```cargo doc  --package rust-in-action-types-array_bin --message-format short --no-deps --open --color always```
///
/// ```cargo test --doc  --package rust-in-action-types-array_bin```
///
/// ## What
