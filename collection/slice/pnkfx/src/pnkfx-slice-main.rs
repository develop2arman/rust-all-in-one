...
/// ## Commands
///
/// ```cargo run -q -p pnkfx-slice_bin --bin pnkfx-slice-main```
///
/// ```cargo doc  --package pnkfx-slice_bin  --message-format short --no-deps --open --color always```
///
/// ```cargo test --doc  --package pnkfx-slice_bin```
///
/// ## What
/// `TODO`
///
...
fn fourth_exercise() {
    ...
    print_contents(&_what_about_me.to_vec());
}
