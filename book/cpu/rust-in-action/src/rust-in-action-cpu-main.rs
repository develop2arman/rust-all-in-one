...
/// ## Commands
///
/// ```RUST_BACKTRACE=full cargo run -q -p rust-in-action-cpu_bin --bin rust-in-action-cpu-main```
///
/// ```cargo doc  --package rust-in-action-cpu_bin  --message-format short --no-deps --open --color always```
///
/// ```cargo test --doc  --package rust-in-action-cpu_bin ```
///
/// ## What
/// `TODO`
///
