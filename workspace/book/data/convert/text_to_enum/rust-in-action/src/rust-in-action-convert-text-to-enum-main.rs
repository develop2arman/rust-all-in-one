...
/// # Commands
///
/// ```cargo run -q -p rust-in-action-convert-text-to-enum_bin --bin rust-in-action-convert-text-to-enum-main```
///
/// ```cargo doc  --package rust-in-action-convert-text-to-enum_bin --message-format short --no-deps --open --color always```
///
/// ```cargo test --doc  --package rust-in-action-convert-text-to-enum_bin```
///
/// ## What
/// `TODO`
///
